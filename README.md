...
- 主更表時間軸：08:00–18:00。
- **分組獨立顯示**：主更表提供「全部」及每個自訂分組的獨立按鈕，點擊後只顯示該分組的崗位及班次。
- **各組編更概況**：主更表及總覽頁顯示每個編更分組的獨立義工人數及已編配總時數。
- 互動拖拉編更：
  - 整段班次水平拖動以改變服務時間。
  ...
  - 時間以 15 分鐘為單位吸附。
- 雙擊更表空白時段可快速新增班次。
- **多義工同時段編配**：新增班次時可一次選擇多名義工；同一崗位的重疊班次會自動分層，崗位列會按同時當值人數增高。
- **午膳時間兩種模式**：
  - 統一時段：所有義工共用同一午膳時間，於主更表以垂直色帶顯示。
  - 分段安排：逐一為每名義工設定午膳開始／結束時間；如午膳落在班次內，會直接在該義工的班次上標示。
